...
app.get(url.parse(OAUTH_PARAMS.callback).path, function(req, res, next) {
  var oauth = {
  ...
  postAsync({ url: url, oauth: oauth })
    .then(function(body) {
      ...
      permData.consumer_key = OAUTH_PARAMS.consumer_key;
      permData.consumer_secret = OAUTH_PARAMS.consumer_secret;
...
function postAsync(params) {
  logger.info("request to " + JSON.stringify(params));
